...
### 主表（7 个 MTL 方法 + 2 个 single_cnt 基线）
...
| single_cnt_5ep | — | — | — | — | 14.20 | 20.25 | 0.764 | — | — | — |
| single_cnt_20ep | — | — | — | — | 12.48 | 17.88 | 0.816 | — | — | — |
| single_cls_5ep | — | — | — | — | — | — | — | 0.986 | **0.9997** | 0.986 |
| vanilla_10ep_ps (unfreeze) | 0.670 | 0.767 | **0.803** | **0.395** | 13.30 | 18.01 | 0.814 | 0.995 | 0.9998 | 0.995 |
| vanilla_frozen_20ep_ps | 0.660 | 0.754 | 0.659 | 0.272 | 12.53 | 17.95 | 0.815 | 0.969 | 0.994 | 0.969 |
| **mtlora_20ep_ps** | **0.677** | **0.775** | 0.773 | 0.354 | 11.51 | 15.97 | 0.853 | **0.998** | **1.0000** | **0.998** |
| tadformer_20ep_ps | 0.672 | 0.770 | 0.643 | 0.256 | 13.26 | 18.42 | 0.805 | 0.976 | 0.995 | 0.975 |
| ditask_20ep_ps | 0.670 | 0.768 | 0.642 | 0.256 | 12.46 | 18.11 | 0.811 | 0.984 | 0.998 | 0.984 |
| pgt_20ep_ps | 0.659 | 0.765 | 0.583 | 0.211 | 12.29 | 18.04 | 0.813 | 0.994 | 0.999 | 0.994 |
| taskprompter_20ep_ps | 0.657 | 0.760 | 0.580 | 0.209 | **10.73** | **15.58** | **0.860** | 0.988 | 0.999 | 0.988 |
...
由 `scripts/compute_delta_m.py` 自动从 `logs/results.csv` 计算（baseline = `single_*_5ep`，按 Δm 降序）：

| 排名 | 方法 | freeze | seg/mIoU | det/AP50 | cnt/MAE | cls/acc | **Δm** |
|---|---|---|---|---|---|---|---|
| 🥇 1 | **mtlora_20ep_ps** | ✅ | +6.72% | -2.34% | +7.79% | +1.21% | **+3.35%** |
| 🥈 2 | vanilla_10ep_ps | ❌ | +5.61% | +1.43% | -6.52% | +0.86% | +0.35% |
| 3 | taskprompter_20ep_ps | ✅ | +3.54% | -26.63% | +14.06% | +0.19% | -2.21% |
| 4 | ditask_20ep_ps | ✅ | +5.56% | -18.86% | +0.24% | -0.24% | -3.33% |
| 5 | vanilla_frozen_20ep_ps | ✅ | +4.01% | -16.71% | -0.40% | -1.71% | -3.70% |
| 6 | pgt_20ep_ps | ✅ | +3.92% | -26.25% | +1.56% | +0.77% | -5.00% |
| 7 | tadformer_20ep_ps | ✅ | +5.90% | -18.73% | -6.20% | -1.09% | -5.03% |

### 关键发现

**1. MTLoRA 是唯一明确正 Δm 的方法（+3.35%）**

7 个 MTL 方法里只有 MTLoRA 实打实拿到正 Δm。Vanilla 几乎打平（+0.35%），剩下 5 个全负。说明在我们这个**异质跨数据集**场景下，绝大多数 MTL 方法实际上是负迁移。

**2. det 是核心分水岭：适配粒度决定抗压能力**
...
**只要 backbone 冻结 + 适配粒度粗（per-stage/block），det 就跌 ~17-27%**。MTLoRA 把这个 drop 限制在 -2.34%（per-Linear 注入细粒度），是它的核心卖点。

**3. cnt 表现整体相近，多数方法持平或略弱单任务**

7 个 MTL 方法 cnt MAE 集中在 10.73-13.30 区间，跟 single_cnt 5ep 的 14.20 相比改进有限（-6.52% ~ +14.06%）。**taskprompter** 在 cnt 上最强（MAE 10.73, R² 0.86），但其他三项任务拖后腿。

cnt 单任务 5ep → 20ep（更充分训练）从 MAE 14.20 → 12.48，**提升 12%**——说明 single_cnt 的"训练量"才是影响指标的主因。如果用 single_cnt_20ep 作为 baseline，所有 MTL 方法的 cnt 项都会从"小正"变成"持平甚至负"——更准确反映"跨任务迁移"本身贡献其实很小。

**4. cls 饱和无信息**

6 类生育期视觉特征差异极大，单任务和 MTL 都拿 99%+，差异在小数第 3-4 位。**这一列不该作为方法对比的依据**，建议换更难数据集或弱化。

**5. 跨数据集 MTL ≠ 论文里的同图 MTL**

TaskPrompter / DiTASK / TADFormer 论文在 NYUD / PASCAL 上都明显赢过 vanilla，但**它们的实验是同一张图做 N 个任务**（共享输入 + 共享 backbone 表示）。我们这里 4 个任务用 **4 个不同数据集**，每个数据集分布不同——backbone-frozen 方法的核心假设"backbone 预训练特征已经够用"在跨域时被打破，所以这些方法表现远低于论文。这是我们项目命题的核心 finding。

**6. Backbone 冻结的代价（"frozen tax"）**

`vanilla` vs `vanilla_frozen` 直接对照：

| | freeze_backbone | det/AP50 | Δm |
|---|---|---|---|
| vanilla | false | 0.803 | +0.35% |
| vanilla_frozen | true | 0.659 | -3.70% |
| 差距 | | **-14.4 pp** | **-4.05 pp** |
